...
if [ ! -z "$(smartctl --scan | grep -v megaraid)" ]; then
  ...
  for i in $DRV; do
    ...
    echo "#" $i
#    smartctl -A $i | grep -i temperature | awk '{printf "%s %24s\t%s\n", $1, $2, $10}'
    smartctl -A $i | awk '{if (($1==190)||($1==194)) printf "%s %24s\t%s\n", $1, $2, $10}'
  done
fi
...
#    DRVROTA=$(echo "$DRVINFO" | grep "Rotation\ Rate" | cut -d':' -f2)
    if [[ "$DRVSNALL" =~ "$DRVSN" ]]; then
      ...
      smartctl -d $(echo $line | awk '{print $3}') -A $(echo $line | awk '{print $1}') | awk '{if (($1==190)||($1==194)) printf "%s %24s\t%s\n", $1, $2, $10}'
    fi
    echo
  done
fi
